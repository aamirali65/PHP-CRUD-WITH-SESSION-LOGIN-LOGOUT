...
<div class="container" style="margin-top: 8rem;">
    <div class="content">
      ...
      if(isset($_POST['btn'])){
        ...
        $email = $_POST['email'];
        $pass = $_POST['pass'];
        $query = mysqli_query($connection,"SELECT * FROM user WHERE Email = '{$email}' AND Password = '{$pass}'");
        if(mysqli_num_rows($query) == 1){
            while($row =  mysqli_fetch_assoc($query)){
                $_SESSION['name'] = $row['Name'];
                $_SESSION['role'] = $row['Role'];
                echo '<script>alert("Login Successfully");window.location.href="http://localhost/crud/show.php"</script>';
            }
            
        }else{
            echo '<script>alert("Login Failed ! Kindly Check Your Email And Password");window.location.href="http://localhost/crud/index.php"</script>';
